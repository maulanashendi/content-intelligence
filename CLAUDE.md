...
`ingest → embed → cluster → label → score`. Each step exposes `python -m <module>.cli run`. Full run: `python -m pipeline.cli run-daily`. Triggered by host cron — no in-app scheduler.

## API endpoints

Read-only, no write side. `/api/v1/clusters/morning`, `/api/v1/clusters/{id}`, `/api/v1/clusters/deferred`, `/api/v1/health`. Auth handled upstream.
...
- No microservices, message queue, Redis cache, separate vector DB, HNSW index, GraphQL, WebSockets, write API, auth code.
