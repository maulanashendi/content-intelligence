...
class Article(Base):
    ...
    embedding: Mapped["ArticleEmbedding | None"] = relationship(back_populates="article", uselist=False)
    gsc_metrics: Mapped[list["ArticleGscMetric"]] = relationship(back_populates="article")
    cluster_memberships: Mapped[list["ArticleClusterMember"]] = relationship(back_populates="article")
...
class ArticleGscMetric(Base):
    ...
    __table_args__ = (
        UniqueConstraint("article_id", "period_start", "period_end", name="uq_gsc_metric_article_period"),
    )
...
class TrendSignalArticle(Base):
    ...
    trend_signal_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("trend_signal.id"), primary_key=True)
...
class ArticleClusterMember(Base):
    ...
    cluster_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("article_cluster.id"), primary_key=True)
    article_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("article.id"), primary_key=True)
...
class ClusterInsight(Base):
    ...
    recommendation: Mapped[InsightRecommendation | None] = mapped_column(Enum(InsightRecommendation))
