...
async def ingest_sitemap(session: AsyncSession, client: httpx.AsyncClient) -> int:
    ...
    for source in sources:
        try:
            ...
            logger.exception("failed to fetch sitemap source=%s url=%s", source.name, source.url)
